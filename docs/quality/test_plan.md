...
工程路径由 `tests/NH/config.local.json` 的 `workspace` 或 `tests/HW/targets.json` 配置，不在计划中保存某台机器的绝对路径。
...
1. 提交代码、测试源码、fixture、runner、配置和本计划后，冻结一个候选提交；最终测试只在该提交上执行。
2. 最终测试开始前，主仓库和八个外部工程必须是已记录的干净工作树，并记录提交、子模块状态及有效配置。
3. HW runner 从主仓库读取完整 Git SHA，通过 Make/CMake overlay 注入 `HW_TEST_LIBRARY_SHA`。工作树不干净时标识追加 `-dirty`；这种结果只可用于预回归，不可作为最终发布证据。
4. F042 Make HW08 不输出普通 marker，其固件身份由 runner metadata、随机非零 `run_id`、归档 ELF 哈希和结果结构共同绑定。
5. 测试必须通过统一 runner 启动。不得直接运行带测试宏的工程 Makefile 代替 HW runner，也不得直接把单个 `run.sh` 的输出作为最终 NH 证据。
6. Make/CMake HW 测试使用纯 overlay，不修改工程的 `main.c`、中断源码、Makefile、`CMakeLists.txt`、linker script 或 `rtt_cfg.h`。
7. 每次切换目标、构建类型、profile 或缓冲参数均执行干净构建。任何代码、runner、fixture、计划或验收判据变更都使受影响证据失效。
8. 自动判定必须检查退出码、结束标识、计数闭环及 Fault/FAIL 禁止项。人工终端观察不能作为唯一结论。
9. 失败后保留原证据，修正测试设施时记录原因并使用新目录重跑；禁止覆盖或改写原始证据。
...
使用 `--evidence-dir PATH` 将同一验收批次写入明确的新目录。`TEST_EVIDENCE/` 默认不提交；发布报告只提交环境、命令、结果和证据索引。
...
1. 被测对象是一个已记录且工作树干净的候选提交；八个外部工程及其库指针全部记录。
...
5. 原始证据目录只读保留，最终报告列出环境、命令、矩阵、失败尝试、有效重跑及证据索引。
