...
- [3.0.0 - 2026-09-09](#300---2026-09-09)
...
- 浮点小数改为固定三位截断；超出 `uint32_t` 整数范围时输出
  ...
  规范为 `0.000`
- 浮点转换路径由编译器宏自动检测改为通过 `RTT_FLOAT_USE_MODFF` 显式选择，
  默认使用不依赖 `modff` 的 IEEE-754 binary32 位解析实现；`modff` 路径作为未来
  动态小数位数、非 IEEE-754 平台和更通用浮点格式化的备用方案保留。开发阶段旧名
  `HARD_FPU_ENABLE` 已移除，继续使用时会触发编译错误
...
- 修复启用 `RTT_LOG_FLOAT_FAST_PATH` 时，legacy 浮点日志的 47 B 标签与负数
  10 位整数部分组合可能使完整帧超过 64 B 局部缓冲区的问题；快速路径现在为
  标签分隔符和最长数值文本预留完整空间，超过 46 B 的标签回退 formatter，
  公开输出保持不变
