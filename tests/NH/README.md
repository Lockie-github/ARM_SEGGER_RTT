...
`run_nh.py` 是 NH01-NH12 的统一入口。它保留各测试用例现有的脚本作为测试实现，并提供配置管理、依赖检查、统一日志和统一证据目录。运行这些测试不需要开发板或调试探针。
...
tests/NH/
  ...
  cases/NH01/ ... NH12/     测试脚本、fixture、配置和资源
  support/                  多个测试用例共用的辅助文件
```

当前每个测试用例均通过 `tests/NH/cases/NHxx/run.sh` 执行；`tests/` 根目录下不再存放 NH 测试源码、fixture、配置或可执行脚本。
...
| NH02 | 分级日志 API | 主机 C 编译器 |
| NH03 | 打印和字符串 API | 主机 C 编译器 |
| NH04 | 格式化行为和 Release 对比 | 主机 C 编译器、Git、tar |
| NH05 | 浮点日志和符号裁剪 | 主机 C 编译器、主机 `nm` |
| NH06 | 功能开关和 Arm 代码裁剪 | 主机 C 编译器、Arm GNU Toolchain |
| NH07 | Make 和 CMake 中的 RTT 配置覆盖 | NH06 所需工具、Make、Ninja、`cube-cmake` |
| NH08 | RTT 写入失败和环形缓冲区行为 | 主机 C 编译器 |
...
命令行选项的优先级高于 JSON 配置值。顶层 `environment` 对象中的环境变量应用于所有测试用例；`cases.NHxx.environment` 中的值只应用于对应的测试用例。
