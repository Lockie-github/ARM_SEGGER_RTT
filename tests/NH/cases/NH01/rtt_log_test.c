...
static void expect_print_output(const char * Body) {
...
static void test_log_string(void) {
  reset_output();
#if !RTT_LOG_ENABLE || !LOG_ENABLE_STRING
  if (log_string("raw text") != 0) {
    fprintf(stderr, "disabled log_string returned an unexpected count\n");
    exit(1);
  }
  expect_output("");
  return;
#else
  {
    int Result = log_string("raw text");
    if (Result != 8) {
      fprintf(stderr, "log_string returned an unexpected count: %d\n", Result);
      exit(1);
    }
  }
  expect_output("raw text");

  reset_output();
  if (log_string("line 1\nline 2") != 13) {
    fprintf(stderr, "log_string did not preserve embedded newline\n");
    exit(1);
  }
  ...
  if (log_string(NULL) != -1) {
    fprintf(stderr, "log_string accepted NULL\n");
    exit(1);
  }
  expect_output("");
  ...
  reset_output();
  ForceWriteFailure = 1;
  if (log_string("full") != 0) {
    fprintf(stderr, "log_string did not report a failed write\n");
    exit(1);
  }
  expect_output("");
#endif
...
static void test_float(void) {
  ...
  memset(LongLabel, 'L', sizeof(LongLabel) - 1u);
  LongLabel[sizeof(LongLabel) - 1u] = '\0';
  snprintf(LongExpected, sizeof(LongExpected), "%s: 1.250\n", LongLabel);
  reset_output();
  log_float_label(LongLabel, 1.25f);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  expect_output(LongExpected);
  if (WriteCount < 2u) {
    fprintf(stderr, "long float label did not use the compatibility formatter\n");
    exit(1);
  }
#else
  expect_output("");
  ...
  reset_output();
  log_float_label("small negative", -0.0004f);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  expect_output("small negative: 0.000\n");
#else
  expect_output("");
#endif

  reset_output();
  log_float_label("boundary negative", -0.0009f);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  expect_output("boundary negative: 0.000\n");
#else
  expect_output("");
#endif

  reset_output();
  log_float_label("retained negative", -0.001953125f);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  expect_output("retained negative: -0.001\n");
#else
  expect_output("");
#endif

  reset_output();
  log_float_label("truncate", 1.9996f);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("small", 0.001953125f);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("negative truncate", -1.9996f);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("above int max", Special.Value);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("below int min", Special.Value);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("overflow", Special.Value);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("negative overflow", Special.Value);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("nan", Special.Value);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("infinity", Special.Value);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  log_float_label("negative infinity", Special.Value);
#if RTT_LOG_ENABLE && LOG_ENABLE_FLOAT
  ...
  reset_output();
  ForceWriteFailure = 1;
  log_float_label("full", Special.Value);
  if (WriteAttemptCount != 1u) {
